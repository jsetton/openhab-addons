...
public class InsteonDevice extends BaseDevice<InsteonAddress, InsteonDeviceHandler> {
    ...
    public void addMissingDeviceLinks() {
        ...
        List<LinkDBRecord> records = getMissingDeviceLinks().values().stream().distinct().toList();
        if (records.isEmpty()) {
            if (logger.isDebugEnabled()) {
            ...
            linkDB.clearPendingChanges();
            records.forEach(linkDB::markRecordForAdd);
            linkDB.update();
        }
    ...
    public void addMissingModemLinks() {
        ...
        List<ModemDBRecord> records = getMissingModemLinks().values().stream().distinct().toList();
        if (records.isEmpty()) {
            if (logger.isDebugEnabled()) {
            ...
            modemDB.clearPendingChanges();
            records.forEach(modemDB::markRecordForAdd);
            modemDB.update();
        }
    }
