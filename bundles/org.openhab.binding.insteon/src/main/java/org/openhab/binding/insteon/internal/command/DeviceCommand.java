...
public class DeviceCommand extends InsteonCommand {
    ...
    private static final String CHANGES_OPTION = "--changes";
    ...
    public List<String> getUsages() {
        return List.of(
                ...
                buildCommandUsage(LIST_DATABASE + " <thingId> [" + CHANGES_OPTION + "]",
                        "list all-link database records or pending changes for a configured Insteon device"),
    ...
    public void execute(String[] args, Console console) {
        ...
        switch (args[0]) {
            ...
            case LIST_DATABASE:
                ...
                } else if (args.length == 3 && CHANGES_OPTION.equals(args[2])) {
                    listDatabasePendingChanges(console, args[1]);
                } else {
                    printUsage(console, args[0]);
                }
                break;
            ...
            case APPLY_DATABASE_CHANGES:
                if (args.length == 3) {
                    applyDatabaseChanges(console, args[1], CONFIRM_OPTION.equals(args[2]));
                } else {
                    printUsage(console, args[0]);
                }
                break;
    ...
    public boolean complete(String[] args, int cursorArgumentIndex, int cursorPosition, List<String> candidates) {
        ...
        if (cursorArgumentIndex == 0) {
            ...
            switch (args[0]) {
                ...
                case ADD_DATABASE_CONTROLLER:
                case DELETE_DATABASE_CONTROLLER:
                    strings = getInsteonDeviceHandlers()
                            .filter(handler -> Optional.ofNullable(handler.getDevice())
                                    .map(device -> !device.getControllerFeatures().isEmpty()).orElse(false))
                ...
                case DELETE_DATABASE_RESPONDER:
                    strings = getInsteonDeviceHandlers()
                            .filter(handler -> Optional.ofNullable(handler.getDevice())
                                    .map(device -> !device.getResponderFeatures().isEmpty()).orElse(false))
                            .map(InsteonDeviceHandler::getThingId).toList();
                    break;
                case APPLY_DATABASE_CHANGES:
                case CLEAR_DATABASE_CHANGES:
                    strings = getInsteonDeviceHandlers()
                            .filter(handler -> Optional.ofNullable(handler.getDevice())
                                    .map(device -> !device.getLinkDB().getPendingChanges().isEmpty()).orElse(false))
                            .map(InsteonDeviceHandler::getThingId).toList();
                    break;
                case LIST_MISSING_LINKS:
                case ADD_MISSING_LINKS:
                ...
                case SET_BUTTON_RADIO_GROUP:
                case CLEAR_BUTTON_RADIO_GROUP:
                    strings = getInsteonDeviceHandlers().filter(handler -> Optional.ofNullable(handler.getDevice())
                            .map(device -> !device.getFeatures(FEATURE_TYPE_KEYPAD_BUTTON).isEmpty()).orElse(false))
                            .map(InsteonDeviceHandler::getThingId).toList();
                    break;
            }
            ...
            switch (args[0]) {
                case LIST_DATABASE:
                    strings = List.of(CHANGES_OPTION);
                    break;
                case ADD_DATABASE_CONTROLLER:
                case ADD_DATABASE_RESPONDER:
                    strings = getModem().getDB().getDevices().stream()
                            .filter(address -> device == null || !device.getAddress().equals(address))
        ...
        if (records.isEmpty()) {
            ...
            console.println("The all-link database for device " + device.getAddress() + " contains " + records.size()
                    + " records:" + (!device.getLinkDB().isComplete() ? " (Partial)"
                            : !device.getLinkDB().getPendingChanges().isEmpty() ? " (Pending Changes)" : ""));
            print(console, records);
        }
    }

    private void listDatabasePendingChanges(Console console, String thingId) {
        InsteonDevice device = getInsteonDevice(thingId);
        if (device == null) {
            console.println("The device " + thingId + " is not configured or enabled!");
            return;
        }
        List<String> changes = device.getLinkDB().getPendingChanges().stream().map(change -> {
            LinkDBRecord record = device.getLinkDB().getRecord(change.getLocation());
            String type = record == null || !record.isActive() ? "ADD" : change.isActive() ? "MODIFY" : "DELETE";
            return change + " (" + type + ")";
        }).toList();
        if (changes.isEmpty()) {
            console.println("The all-link database for device " + device.getAddress() + " has no pending changes");
        } else {
    ...
    private void addMissingLinks(Console console, String thingId) {
        ...
        if (device == null) {
            ...
            console.println("The link database for device " + thingId + " is not loaded yet.");
        } else if (!device.getLinkDB().getPendingChanges().isEmpty()) {
            console.println("The link database for device " + thingId + " has pending changes.");
        } else if (!getModem().getDB().isComplete()) {
            console.println("The modem database is not loaded yet.");
        } else if (!getModem().getDB().getPendingChanges().isEmpty()) {
            console.println("The modem database has pending changes.");
        } else {
            Map<String, LinkDBRecord> deviceLinks = device.getMissingDeviceLinks();
            Map<String, ModemDBRecord> modemLinks = device.getMissingModemLinks();
            if (deviceLinks.isEmpty() && modemLinks.isEmpty()) {
                console.println("There are no missing links for device " + device.getAddress() + ".");
            } else {
                if (!deviceLinks.isEmpty()) {
                    if (!device.isAwake() || !device.isResponding()) {
                        console.println("Scheduling the missing links for device " + device.getAddress()
                                + " to be added to its link database the next time it is "
                                + (device.isBatteryPowered() ? "awake" : "responding") + ".");
                    } else {
                        console.println("Adding the missing links for device " + device.getAddress()
                ...
                if (!modemLinks.isEmpty()) {
                    console.println(
                            "Adding the missing links for device " + device.getAddress() + " to the modem database...");
    ...
    private void addDatabaseRecord(Console console, String[] args, boolean isController) {
        ...
        if (device == null) {
            ...
            if (record == null) {
                console.println(
                        "Added a pending change to add link database record for device " + device.getAddress() + ".");
            } else {
    ...
    private void deleteDatabaseRecord(Console console, String[] args, boolean isController) {
        ...
        if (device == null) {
            ...
            if (record == null) {
                console.println("No link database record found to delete for device " + device.getAddress() + ".");
            } else {
    ...
    private void applyDatabaseChanges(Console console, String thingId, boolean isConfirmed) {
        ...
        if (device == null) {
            ...
            listDatabasePendingChanges(console, thingId);
            console.println("Please run the same command with " + CONFIRM_OPTION
                    + " option to have the listed changes written to the link database for device "
                    + device.getAddress() + ".");
        } else {
            if (!device.isAwake() || !device.isResponding() || !getModem().getDB().isComplete()) {
                console.println("Scheduling the pending changes for device " + device.getAddress()
                        + " to be applied to its link database the next time it is "
                        + (device.isBatteryPowered() ? "awake" : "responding") + ".");
            } else {
                console.println(
                        "Applying the pending changes to link database for device " + device.getAddress() + "...");
            }
    ...
    private void clearDatabaseChanges(Console console, String thingId) {
        ...
        if (device == null) {
            ...
            device.getLinkDB().clearPendingChanges();

            console.println("Cleared the pending changes from link database for device " + device.getAddress() + ".");
        }
    }
    ...
    private void setButtonRadioGroup(Console console, String[] args) {
        ...
        if (device == null) {
            ...
            console.println("Setting radio group for device " + device.getAddress() + "...");
    ...
    private void clearButtonRadioGroup(Console console, String[] args) {
        ...
        if (device == null) {
            ...
            console.println("Clearing radio group for device " + device.getAddress() + "...");
