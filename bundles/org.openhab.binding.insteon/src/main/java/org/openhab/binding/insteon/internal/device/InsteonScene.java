...
public class InsteonScene {
    ...
    public void addDeviceFeature(InsteonDevice device, int onLevel, @Nullable RampRate rampRate, int componentId) {
        ...
        modem.getDB().clearPendingChanges();
        modem.getDB().markRecordForAdd(device.getAddress(), group, true);
        modem.getDB().update();

        device.getLinkDB().clearPendingChanges();
        device.getLinkDB().markRecordForAdd(modem.getAddress(), group, false, new byte[] { (byte) onLevel,
    ...
    public void removeDeviceFeature(InsteonDevice device, int componentId) {
        ...
        modem.getDB().clearPendingChanges();
        modem.getDB().markRecordForDelete(device.getAddress(), group, true);
        modem.getDB().update();
