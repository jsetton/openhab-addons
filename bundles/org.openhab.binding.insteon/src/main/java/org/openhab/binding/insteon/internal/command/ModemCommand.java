...
public class ModemCommand extends InsteonCommand {
    ...
    private static final List<String> SUBCMDS = List.of(LIST_ALL, LIST_DATABASE, RELOAD_DATABASE,
            ADD_DATABASE_CONTROLLER, ADD_DATABASE_RESPONDER, DELETE_DATABASE_CONTROLLER, DELETE_DATABASE_RESPONDER,
            APPLY_DATABASE_CHANGES, CLEAR_DATABASE_CHANGES, ADD_DEVICE, REMOVE_DEVICE, SWITCH);
    ...
    public List<String> getUsages() {
        return List.of(
                ...
                buildCommandUsage(LIST_DATABASE + " [" + RECORDS_OPTION + "|" + CHANGES_OPTION + "]",
                        "list all-link database summary, records or pending changes for the Insteon modem"),
                ...
                buildCommandUsage(DELETE_DATABASE_CONTROLLER + " <address> <group>",
                        "delete a controller record from all-link database for the Insteon modem"),
                buildCommandUsage(DELETE_DATABASE_RESPONDER + " <address> <group>",
                        "delete a responder record from all-link database for the Insteon modem"),
    ...
    public void execute(String[] args, Console console) {
        ...
        switch (args[0]) {
            ...
            case LIST_DATABASE:
                ...
                } else if (args.length == 2 && CHANGES_OPTION.equals(args[1])) {
                    listDatabasePendingChanges(console);
                } else {
                    printUsage(console, args[0]);
                }
                break;
            ...
            case APPLY_DATABASE_CHANGES:
                if (args.length == 2) {
                    applyDatabaseChanges(console, CONFIRM_OPTION.equals(args[1]));
                } else {
                    printUsage(console, args[0]);
                }
                break;
    ...
    public boolean complete(String[] args, int cursorArgumentIndex, int cursorPosition, List<String> candidates) {
        ...
        if (cursorArgumentIndex == 0) {
            ...
            switch (args[0]) {
                case LIST_DATABASE:
                    strings = List.of(RECORDS_OPTION, CHANGES_OPTION);
                    break;
                case ADD_DATABASE_CONTROLLER:
                case ADD_DATABASE_RESPONDER:
                case REMOVE_DEVICE:
                    ...
                    break;
                case DELETE_DATABASE_CONTROLLER:
                    strings = getModem().getDB().getRecords().stream().filter(ModemDBRecord::isController)
                            .map(record -> record.getAddress().toString()).distinct().toList();
                    break;
                case DELETE_DATABASE_RESPONDER:
                    strings = getModem().getDB().getRecords().stream().filter(ModemDBRecord::isResponder)
                            .map(record -> record.getAddress().toString()).distinct().toList();
                    break;
                case SWITCH:
            ...
            switch (args[0]) {
                case DELETE_DATABASE_CONTROLLER:
                    if (address != null) {
                        strings = getModem().getDB().getRecords(address).stream().filter(ModemDBRecord::isController)
                                .map(record -> HexUtils.getHexString(record.getGroup())).distinct().toList();
                    }
                    break;
                case DELETE_DATABASE_RESPONDER:
                    if (address != null) {
                        strings = getModem().getDB().getRecords(address).stream().filter(ModemDBRecord::isResponder)
                                .map(record -> HexUtils.getHexString(record.getGroup())).distinct().toList();
                    }
                    break;
                case REMOVE_DEVICE:
    ...
    private void listDatabasePendingChanges(Console console) {
        InsteonAddress address = getModem().getAddress();
        List<String> changes = getModem().getDB().getPendingChanges().stream().map(change -> {
            ModemDBRecord record = getModem().getDB().getRecord(change.getAddress(), change.getGroup(),
                    change.isController());
        ...
        if (InsteonAddress.UNKNOWN.equals(address)) {
            ...
            console.println("The all-link database for modem " + address + " has no pending changes");
        } else {
            console.println(
    ...
    private void addDatabaseRecord(Console console, String[] args, boolean isController) {
        if (!getModem().getDB().isComplete()) {
            ...
            if (record == null) {
                console.println("Added a pending change to add a modem database record.");
            } else {
                console.println("Added a pending change to modify a modem database record.");
            }
        }
    }
    ...
    private void deleteDatabaseRecord(Console console, String[] args, boolean isController) {
        if (!getModem().getDB().isComplete()) {
            ...
            ModemDBRecord record = getModem().getDB().getRecord(address, group, isController);
            if (record == null) {
                console.println("No modem database record found to delete.");
            } else {
                getModem().getDB().markRecordForDelete(record);

                console.println("Added a pending change to delete a modem database record.");
            }
        }
    }
    ...
    private void applyDatabaseChanges(Console console, boolean isConfirmed) {
        if (!getModem().getDB().isComplete()) {
            ...
            console.println("Please run the same command with " + CONFIRM_OPTION
                    + " option to have the listed changes written to the modem database.");
        } else {
            console.println("Applying the pending changes to the modem database...");
            getModem().getDB().update();
        }
    }

    private void clearDatabaseChanges(Console console) {
        if (getModem().getDB().getPendingChanges().isEmpty()) {
            console.println("The modem database has no pending changes.");
        } else {
            getModem().getDB().clearPendingChanges();
            console.println("Cleared the pending changes from the modem database.");
        }
    }
