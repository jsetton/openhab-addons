...
public class ModemDBRecord extends DatabaseRecord {
    ...
    /**
     * Factory method for creating a new ModemDBRecord from another isntance with new type
     *
     * @param type the new record type to use
     * @param record the modem db record to use
     * @return the modem db record with new type
     */
    public static ModemDBRecord withNewType(RecordType type, ModemDBRecord record) {
        return new ModemDBRecord(type, record.getGroup(), record.getAddress(), record.getData());
    }
}
