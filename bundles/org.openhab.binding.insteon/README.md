...
The modem and device link databases are only downloaded once unless the binding receives an indication that a database was updated or marked to be refreshed via the openHAB console.
...
The device type is automatically determine by the binding using the device product data.
...
The binding provides commands you can use to help with configuring and troubleshooting.
...
These groups can be linked programmatically using the `insteon device addMissingLinks` console command, or via the device set buttons (see instructions that come with the keypad).
...
An `ON` state indicates that all the device states associated to a scene are matching the configured link on level.
